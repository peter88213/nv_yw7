...
class NovxFile(File):
    ...
    def read(self):
        ...
        try:
            locale = root.attrib['{http://www.w3.org/XML/1998/namespace}lang']
        ...
        xmlArcs = root.find('ARCS')
        self._read_arcs(xmlArcs)
        self.adjust_scene_types()

        xmlWclog = root.find('PROGRESS')
        if xmlWclog is not None:
            for xmlWc in xmlWclog.findall('WC'):
    ...
    def write(self):
        ...
        root = ET.Element('novx', attrib=attrib)
        self._build_element_tree(root)
        indent(root)
